...
struct Segment {
  ...
  inline double dis(Segment &s) {
    Point p(r1.line.intersection(s.r1.line));
    if (contains(p) && s.contains(p)) {
      return 0;
    }
    return std::min({dis(s.p1), dis(s.p2), s.dis(p1), s.dis(p2)});
  }
};

signed main() {
  Point st1(true), p1(true), st2(true), p2(true);
  Segment r1(st1, p1), r2(st2, p2);
  fprintf(OUT, "%.10lf\n", r1.dis(r2));
  return 0;
}
